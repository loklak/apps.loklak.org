...
function parser(data) {
    ...
    for (var word in words) {
        if (words[word].startsWith("@")) {
            ...
            parsed += "<a href='https://twitter.com/hashtag/" + words[word].slice(1) + "' target='_blank'>" +
              words[word] + "</a> ";
        } else if (words[word].startsWith("http")) {
            if (words[word].startsWith("http://loklak")) {
                parsed += "<a href='" + data.statuses[tweetNum].links[loklakLinkCount] + "' target='_blank'>" +
                  data.statuses[tweetNum].links[loklakLinkCount] + "</a> ";
                loklakLinkCount += 1;
            } else {
    ...
    document.getElementById("tweet-info").innerHTML = "Posted by <a href='https://twitter.com/" +
      user.screen_name + "'><b>" + user.name + "</b></a>, <a href='" + data.statuses[tweetNum].link +
      "'><i>" + tweetDate.toUTCString() + "</i></a>";
